class Api::TimelineController < ApplicationController
  ...
  def index
    ...
    @target_posts = Post.includes({ poster: :profile_picture }, :target, comments: { commenter: :profile_picture }).where("target_id IN (?) OR poster_id IN (?)", @timeline_ids, @timeline_ids)
